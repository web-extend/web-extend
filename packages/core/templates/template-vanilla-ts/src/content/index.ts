...
function render(root: HTMLElement) {
  root.innerHTML = `<div class="my-content-container">
    <div class="my-content" style="display: none;">
      <h1>Hello World</h1>
  ...
  const handleClick = () => {
    const el = document.getElementsByClassName('my-content')[0] as HTMLElement;
    if (el) {
      el.style.display = el.style.display === 'none' ? 'block' : 'none';
    }
  };
  const btn = document.querySelector('.my-content-container svg');
